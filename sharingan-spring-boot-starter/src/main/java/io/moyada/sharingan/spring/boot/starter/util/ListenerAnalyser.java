...
import io.moyada.sharingan.spring.boot.starter.annotation.Rename;
import io.moyada.sharingan.spring.boot.starter.context.ListenerInfo;
import io.moyada.sharingan.spring.boot.starter.context.ListenerMethod;
import io.moyada.sharingan.spring.boot.starter.context.ProxyField;
import io.moyada.sharingan.spring.boot.starter.util.VariableUtil;
...
public class ListenerAnalyser {
    ...
    public static ListenerInfo getListenerInfo(Class<?> targetClass) {
        if (!targetClass.isAnnotationPresent(io.moyada.sharingan.spring.boot.starter.annotation.Monitor.class)) {
            return null;
        }
        ...
        io.moyada.sharingan.spring.boot.starter.annotation.Monitor annotation = targetClass.getAnnotation(io.moyada.sharingan.spring.boot.starter.annotation.Monitor.class);
    ...
    private static List<ListenerMethod> getListenerMethod(Class<?> clazz) {
        ...
        for (Method method : methods) {
            ...
            if (method.isAnnotationPresent(io.moyada.sharingan.spring.boot.starter.annotation.Register.class)) {
                listenerMethod.setNeedRegister();
                listenerMethod.setHttpData(method.getAnnotation(io.moyada.sharingan.spring.boot.starter.annotation.HttpMethod.class));
            }
            listenerMethods.add(listenerMethod);

            io.moyada.sharingan.spring.boot.starter.annotation.Listener value = method.getAnnotation(io.moyada.sharingan.spring.boot.starter.annotation.Listener.class);
            if (null == value) {
                continue;
            }
    ...
    private static boolean needListener(Method method) {
        ...
        if (!method.isAnnotationPresent(io.moyada.sharingan.spring.boot.starter.annotation.Listener.class) && !method.isAnnotationPresent(io.moyada.sharingan.spring.boot.starter.annotation.Register.class)) {
            return false;
        }
    ...
    private static List<ProxyField> getListenerInfo(Class clazz, Method method) {
        ...
        for (int index = 0; index < length; index++) {
            ...
            if (parameter.isAnnotationPresent(io.moyada.sharingan.spring.boot.starter.annotation.Exclusive.class)) {
                continue;
            }
            ...
            io.moyada.sharingan.spring.boot.starter.annotation.Rename rename = parameter.getAnnotation(Rename.class);
            if (null == rename) {
